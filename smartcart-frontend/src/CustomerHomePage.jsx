...
export default function CustomerHomePage() {
  ...
  const fetchProducts = async (category = '') => {
    try {
      const response = await fetch(
        `http://localhost:8081/api/products${category ? `?category=${category}` : '?category=Shirts'}`, 
  ...
  const fetchCartCount = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8081/api/cart/items/count?username=${username}`, {
        credentials: 'include', // Include authToken as a cookie
      });
